...
ResultSet rs=stmt.executeQuery("SELECT DATATIME1, round(decode(SUM(BHGPS),0,0,(SUM(BHGPS)/(sum(JCPS*5)))*100),2)AS PCFAI不良率,round(decode(SUM(NONBHGPS),0,0,(SUM(NONBHGPS)/(sum(NONJCPS*5)))*100),2)AS NonPCFAI不良率,round(decode(SUM(BLJS),0,0,(SUM(BLJS)/(sum(GDS)))*100),2)AS PC异常比例,round(decode(SUM(NONBLJS),0,0,(SUM(NONBLJS)/(sum(NONGDS)))*100),2)AS NonPC异常比例 from WIQWMS.MQA where DATATIME1 BETWEEN to_CHAR('"+startime+"')AND to_char('"+endtime+"') GROUP BY DATATIME1 ORDER BY DATATIME1");
...
ResultSet qd=ccdd.executeQuery("SELECT DATATIME2, round(decode(SUM(WGBLS),0,0,(SUM(WGBLS)/SUM(LJCYS)*1000000)),2)AS PC外观DPPM,round(decode(SUM(NONWGBLS),0,0,(SUM(NONWGBLS)/SUM(NONLJCYS)*1000000)),2)AS NonPC外观DPPM,round(decode(SUM(WGBLS+NONWGBLS),0,0,(SUM(WGBLS+NONWGBLS)/SUM(LJCYS+NONLJCYS)*1000000)),2)AS total外观DPPM from WIQWMS.OOBA where DATATIME2 BETWEEN to_CHAR('"+startime+"')AND to_char('"+endtime+"') GROUP BY DATATIME2 ORDER BY DATATIME2");
ResultSet cbb=cb.executeQuery("SELECT DATATIME2, round(decode(SUM(DXBLS),0,0,(SUM(DXBLS)/SUM(LJCYS)*1000000)),2)AS PC电性DPPM,round(decode(SUM(NONDXBLS),0,0,(SUM(NONDXBLS)/SUM(NONLJCYS)*1000000)),2)AS NonPC电性DPPM,round(decode(SUM(DXBLS +NONDXBLS),0,0,(SUM(DXBLS +NONDXBLS)/SUM(LJCYS +NONLJCYS)*1000000)),2)AS total电性DPPM from WIQWMS.OOBA where DATATIME2 BETWEEN to_CHAR('"+startime+"')AND to_char('"+endtime+"') GROUP BY DATATIME2 ORDER BY DATATIME2");
...
%>
	...
	<table id="jzsl" style="border-collapse: collapse;text-align: center;font-size:0.8em;width:130%;">
	...
	<div class="xpanel xpanel-c-b" style="margin-top: 13%;">
		<div class="title">汇总数据表</div>
